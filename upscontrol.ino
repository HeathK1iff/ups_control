...
char* getSensorId(char *buf, char *prefix, uint32_t chipId){
  sprintf(buf, "%s_%d", prefix, chipId);
  return buf;
}
...
void callAPISensor(){
  ...
  if (server.args() > 0) {
    if (server.hasArg(API_SENSOR_ID)) {
      String sel_sensor = server.arg(API_SENSOR_ID);
      ...
      for (int i = 0; i < deviceCount; i++) {
        ...
        if (strcmp(sensorAddr, sel_sensor.c_str()) == 0){
          out[SENSOR_ID] = sel_sensor;
          out[SENSOR_TYPE] = SENSOR_TYPE_TEMP;
          out[SENSOR_VAL] = sensors.getTempCByIndex(i);
          break;
        }
      }

      if (upsinfo.isConnectedUps()){
        if (strcmp(getSensorId(bufId, SENSOR_VOLT, ESP.getChipId()), sel_sensor.c_str()) == 0){
          out[SENSOR_ID] = sel_sensor.c_str();
          out[SENSOR_TYPE] = SENSOR_TYPE_POWER_VOLTAGE;
          if (!upsinfo.getStatus().option.UtilityFail){
            out[SENSOR_VAL] = upsinfo.getIpVoltage();
          } else {
            out[SENSOR_VAL] = 0;
          }
        }
        memset(bufId, '\0', 50);
        if (strcmp(getSensorId(bufId, SENSOR_FREQ, ESP.getChipId()), sel_sensor.c_str()) == 0){
          out[SENSOR_ID] = sel_sensor.c_str();
          out[SENSOR_TYPE] = SENSOR_TYPE_POWER_FREQUENCY;
          out[SENSOR_VAL] = upsinfo.getIpFrequency();
        }
      }
    }
  } else {  
    ...
    if (upsinfo.isConnectedUps()){
      ...
      volt[SENSOR_ID] = getSensorId(bufId, SENSOR_VOLT, ESP.getChipId());
      ...
      freq[SENSOR_ID] =   getSensorId(bufId, SENSOR_FREQ, ESP.getChipId());
